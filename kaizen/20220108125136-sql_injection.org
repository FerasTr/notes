...
* Wildcard Injection
Some databases accept wildcards such as the asterisk ~*~ which matches all entries in a table. To find credentials in this case, we can write a scirpt that appends ~*~ to the username/password and collect all characters that result in valid requests.
