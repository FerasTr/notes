...
- In the Oracle database, every ~SELECT~ statement must specify a table to select ~FROM~. /dual/ is a built-in database in Oracle which can be used for this purpose.
