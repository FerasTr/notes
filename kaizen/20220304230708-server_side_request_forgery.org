...
#+end_src
** Blind SSRF
When the application can be tricked to issue an HTTP request to a supplied URL, but the response of that request is not present in the front-end response of the server, then this is called blind server-side request forgery. Blind SSRF makes it harder to leak sensitive data, since the content of the response is not shown to the attacker.

Detecting blind SSRF can be done by using an external server that the attacker controls. When the vulnerable server makes an HTTP request to the server for the first time, a [[id:c620b007-1376-4f39-9808-5ff9ff487e93][DNS]] lookup is first made. The attacker can monitor the server for any network communication such as HTTP or DNS. In some cases only the DNS lookup is completed since some infrastructures block outgoing HTTP traffic to unknown domains.

** Circumventing common SSRF Defenses
*** Blacklist Approach
Some applications will block specific IP addresses such as 127.0.0.1 and the hostname /localhost/. Such defenses can be bypassed by using an alternative representation of the required IP address. Or by registering a domain that resolves to 127.0.0.1.
...
- OWASP's artciles on ssrf ([[https://owasp.org/Top10/A10_2021-Server-Side_Request_Forgery_%28SSRF%29/][1]], [[https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html][2]])
